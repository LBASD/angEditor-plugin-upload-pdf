...
import { PDF_SVG } from '../../constants/icon-svg'
import { IUploadConfigForPDF } from './config'
import $ from '../../utils/dom'
import { insertPDF, uploadPDF } from './helper'

class UploadPDFMenu implements IButtonMenu {
  readonly title = t('pdf.upload')
  readonly iconSvg = PDF_SVG
  readonly tag = 'button'
  ...
  exec(editor: IDomEditor, value: string | boolean) {
    ...
    if (customBrowseAndUpload) {
      customBrowseAndUpload((src: string, shortcode: string) => insertPDF(editor, src, shortcode))
      return
    }
  ...
  private getMenuConfig(editor: IDomEditor): IUploadConfigForPDF {
    // 获取配置，见 `./config.js`
